...
# 将数据转移到 GPU 上
mu = cp.random.randn(n_xy, n_xy, n_z)
mu = cp.clip(mu, -1.0, 1.0)
phi = mu.copy()
temp = phi.copy()
...
for it in trange(1, 6600, desc="outer_loop", leave=True):
    phi_pr = phi.copy()
    for ik in trange(50, desc="inner_loop", leave=False):
        ...
        # 计算A的逆矩阵
        A_inv = cp.zeros((2, 2, n_xy - 2, n_xy - 2, n_z - 2))
        A_inv[0, 0] = d / det_A
        A_inv[0, 1] = -b / det_A
        A_inv[1, 0] = -c / det_A
        A_inv[1, 1] = a / det_A
    ...
    if it % 66 == 1:
        # 将数据从 GPU 转移到 CPU 上进行绘图
        temp = cp.asnumpy(phi)
        # 使用 marching_cubes 提取等值面
        verts, faces, _, _ = measure.marching_cubes(temp[1:-1,1:-1,1:-1], 0)
        # 调整顶点坐标到实际的网格坐标
        ...
        plt.savefig("3dCahnHilliard_CUBE/time={:.4f}_.png".format(it * dt), dpi=300)
        plt.close(fig)
